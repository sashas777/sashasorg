...
$(function(){
	if($('.pro_tweet').length)$.include('/public/js/jquery.tweet.js');
	if($('.lightbox-image').length)$.include('/public/js/jquery.prettyPhoto.js');
	if($('#pro_contact-form').length||$('#contact-form').length)$.include('/public/js/forms.js');
	if($('.pro_kwicks').length)$.include('/public/js/kwicks-1.5.1.pack.js');
	if($('#pro_counter').length)$.include('/public/js/jquery.countdown.js');
	if($('.fixedtip').length||$('.clicktip').length||$('.normaltip').length)$.include('/public/js/jquery.atooltip.pack.js')
...
$('.gallery')._TMS({
	...
	duration:1000,
	preset:'simpleFade',
	pagination:$('.img-pags').uCarousel({show:4}),
	slideshow:5000,
	banners:'fade',// fromLeft, fromRight, fromTop, fromBottom
	waitBannerAnimation:false,
	progressBar:false})			

$('#news a').hover(function(){$(this).stop().animate({opacity:.3})},(function(){$(this).stop().animate({opacity:1})}));
